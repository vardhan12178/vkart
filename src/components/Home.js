  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-gray-50">
        ...
        <div className="hidden lg:flex absolute bottom-3 right-3 translate-y-12 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
      ...
      <AnimatePresence>
        {show2faNudge && (
          <motion.div 
            ...
            className="fixed bottom-6 right-6 z-50 w-[90%] max-w-sm bg-white/90 backdrop-blur-2xl border border-orange-100 shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-2xl p-5 flex gap-4"
          >
      ...
      <section className="py-24 bg-white">
         <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
               {loading ? [1,2,3,4].map(i => <SkeletonCard key={i} />) : newArrivals.map(p => <ProductCard key={p._id} p={p} />)}
            </div>
         </div>
      </section>
