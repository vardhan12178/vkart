...
const Clothing = ({ category }) => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    if (categoryFilter) {
      url = `https://fakestoreapi.com/products/category/${categoryFilter}?limit=10&page=${page}`;
    }
    try {
      ...
      setHasMore(data.length === 10); // Assume more if we get full limit
    } catch (error) {
      console.error('Error fetching products:', error);
      setHasMore(false);
    }
  }, [sortOption, categoryFilter, sortOrder, page]);

  useEffect(() => {
    setPage(1); // Reset page when dependencies change
    fetchProducts();
  }, [fetchProducts, searchTerm]); // Add searchTerm to trigger re-fetch
...
export const MenClothing = () => <Clothing category="men's clothing" />;
export const WomenClothing = () => <Clothing category="women's clothing" />;
