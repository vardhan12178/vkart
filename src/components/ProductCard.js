...
import {
  ...
  FaCheckCircle,
  FaUserCircle,
  FaCube, // Added icon for 3D view
  FaImages // Added icon for Image view
} from "react-icons/fa";
...
export default function ProductCard() {
  ...
  useEffect(() => {
    ...
    setNav2(null);
    setQuantity(1);
    setViewMode("images"); // Reset to images when changing products
  }, [id]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-7 space-y-6 animate-fade-up" style={{ animationDelay: '0.1s' }}>
             <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100 relative">
                ...
                {/* --- 3D / IMAGE TOGGLE BUTTONS --- */}
                <div className="absolute top-6 right-6 z-30 flex bg-white/90 backdrop-blur rounded-full p-1 shadow-md border border-gray-100">
                   <button 
                     onClick={() => setViewMode("images")}
                     className={`p-2 rounded-full transition-all ${viewMode === "images" ? "bg-gray-900 text-white" : "text-gray-500 hover:bg-gray-100"}`}
                     title="View Images"
                   >
                     <FaImages size={14} />
                   </button>
                   <button 
                     ...
                     className={`p-2 rounded-full transition-all ${viewMode === "3d" ? "bg-gray-900 text-white" : "text-gray-500 hover:bg-gray-100"}`}
                     title="View 3D Model"
                   >
                     <FaCube size={14} />
                   </button>
                ...
                <div className="relative group rounded-2xl overflow-hidden bg-gray-50 aspect-square md:aspect-[4/3]">
                   {viewMode === "3d" ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                         {/* Assuming ProductModel takes the product ID or Model URL */}
                ...
                {/* Thumbnails (Only visible in Image Mode) */}
                {viewMode === "images" && imgs.length > 1 && (
            ...
            <div ref={buyBoxRef} className="space-y-6">
               <div className="flex gap-4">
                  ...
                  <button
                    onClick={handleAdd}
                    disabled={stock === 0}
                    className={`flex-1 h-14 rounded-xl font-bold text-lg shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center justify-center gap-2 ${
                      stock === 0 
               ...
               <div className="flex gap-4">
                  <button 
                    onClick={handleWish}
                    className={`flex-1 h-12 rounded-xl font-bold border flex items-center justify-center gap-2 transition-all text-sm ${
                      wish ? "border-rose-200 bg-rose-50 text-rose-600" : "border-gray-200 text-gray-600 hover:border-gray-300 hover:bg-gray-50"
                    }`}
                  >
